...
| com.xx.app_name.adapter) | 存放app所有的適配器類 |
...
小駝峰命名法。命名風格通常都是XX_selector、XX_drawable、XX_layer。
...
return 'ufeff' + content; // byte order mark | Good，對於非打印字符，使用轉義，並在必要時寫上註釋
...
標識符只能使用ASCII字母和數字，因此每個有效的標識符名稱都能匹配正則表達式w+。
